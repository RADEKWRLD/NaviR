...
// 点击事件处理
document.addEventListener('click', function(e) {
    // 如果点击的是搜索框或搜索相关元素，不触发快捷方式菜单
    const searchElements = ['search-input', 'search-form', 'engine-btn', 'engine-menu', 'search-btn'];
    if (searchElements.some(id => e.target.id === id || e.target.closest(`#${id}`))) {
        return;
    }
...
function loadShortcutNewTabSetting() {
    return localStorage.getItem('shortcut_newtab') === 'true';
}
function saveShortcutNewTabSetting(val) {
    localStorage.setItem('shortcut_newtab', val ? 'true' : 'false');
}
if (shortcutNewTabCheckbox) {
    shortcutNewTabCheckbox.checked = loadShortcutNewTabSetting();
    shortcutNewTabCheckbox.addEventListener('change', function() {
        saveShortcutNewTabSetting(this.checked);
    });
}
...
function createShortcutElement(shortcut) {
    ...
    div.addEventListener('click', (e) => {
        if (!e.target.classList.contains('delete-shortcut')) {
            const openInNewTab = loadShortcutNewTabSetting();
            if (openInNewTab) {
...
function handleAddShortcut() {
    ...
    async function fetchPageTitle(url) {
        ...
        try {
            ...
            if (commonSites[hostname]) {
                return commonSites[hostname];
            }
            ...
            for (const proxyUrl of proxyServices) {
                try {
                    ...
                    console.log(`Proxy service failed: ${proxyUrl}`, error);
                    continue;
                }
            }
        } catch (error) {
            console.error('Error in fetchPageTitle:', error);
        }
        ...
        if (!title) {
            try {
                ...
                title = url_obj.hostname.replace('www.', '');
                // 将域名中的连字符和点替换为空格，并将首字母大写
                title = title
                    .split(/[-.]/)
    ...
    async function addShortcut() {
        ...
        const defaultIcon = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10z"/></svg>';
    ...
    urlInput.addEventListener('input', async () => {
        ...
        const hint = titleInput.parentElement.querySelector('.auto-fetch-hint');
        saveBtn.disabled = true; // 重置时禁用保存按钮
        
        if (!titleInput.value.trim()) {  // 只在标题输入框为空时自动获取
            titleFetchTimeout = setTimeout(async () => {
                ...
                if (url) {
                    ...
                    // 显示加载提示
                    if (hint) {
                        hint.textContent = languageManager.getTranslation('shortcut.add.fetching');
                    ...
                    try {
                        ...
                        if (title) {
                            ...
                            saveBtn.disabled = false; // 获取成功后启用保存按钮
                        } else {
                            if (hint) {
                                hint.textContent = languageManager.getTranslation('shortcut.add.fetch_failed');
                                hint.style.color = 'red';
                            }
                            saveBtn.disabled = true; // 获取失败时禁用保存按钮
                        }
                    } catch (err) {
                        if (hint) {
                            hint.textContent = languageManager.getTranslation('shortcut.add.fetch_failed');
                        ...
                        saveBtn.disabled = true; // 发生错误时禁用保存按钮
                    }
                }
            }, 1000);
        } else {
...
function initLanguageToggle() {
    ...
    if (savedLang) {
        const savedLangBtn = document.querySelector(`.language-btn[data-lang="${savedLang}"]`);
        if (savedLangBtn) {
            languageBtns.forEach(b => b.classList.remove('active'));
...
function initOpacityControl() {
    ...
    // 从本地存储加载透明度设置
    const savedOpacity = localStorage.getItem('background_opacity') || '0';
    body.style.backgroundColor = `rgba(0, 0, 0, ${savedOpacity})`;
    contextMenu.style.background = `rgba(0, 0, 0, ${savedOpacity})`;
    
    // 设置保存的按钮状态
    opacityBtns.forEach(btn => {
        if (btn.getAttribute('data-opacity') === savedOpacity) {
            btn.classList.add('active');
        } else {
    ...
    opacityBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // 更新按钮状态
            opacityBtns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            
            // 更新背景透明度
            body.style.backgroundColor = `rgba(0, 0, 0, ${opacity})`;
            contextMenu.style.background = `rgba(0, 0, 0, ${opacity})`;
