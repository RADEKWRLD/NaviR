...
// 设置主区域的初始样式
main.style.willChange = 'transform, filter';
...
function showContextMenu() {
    if (isMenuVisible) return; // 防止重复触发
    isMenuVisible = true;
    
    // 使用 CSS 类来管理状态，减少直接样式操作
    main.classList.add('context-active');
    ...
    requestAnimationFrame(() => {
        ...
        gsap.to(contextMenu, { 
            ...
            duration: 0.3, 
            ease: 'power1.out',
            clearProps: 'transform' // 动画结束后清理属性
        });
...
function hideContextMenu() {
    ...
    // 移除状态类
    main.classList.remove('context-active');
