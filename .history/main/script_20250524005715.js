...
// 搜索引擎使用记录
const SEARCH_ENGINE_USAGE_KEY = 'naviR_search_engine_usage';

// 初始化搜索引擎使用记录
function initSearchEngineUsage() {
    const usage = localStorage.getItem(SEARCH_ENGINE_USAGE_KEY);
    return usage ? JSON.parse(usage) : {};
}

// 记录搜索引擎使用
function recordSearchEngineUsage(engineName) {
    const usage = initSearchEngineUsage();
    ...
    usage[today][engineName] = (usage[today][engineName] || 0) + 1;
    localStorage.setItem(SEARCH_ENGINE_USAGE_KEY, JSON.stringify(usage));
}

// 获取搜索引擎使用统计
function getSearchEngineStats(days = 7) {
    const usage = initSearchEngineUsage();
    const stats = {};
    ...
    for (let i = 0; i < days; i++) {
        ...
        if (usage[dateStr]) {
            Object.entries(usage[dateStr]).forEach(([engine, count]) => {
                stats[engine] = (stats[engine] || 0) + count;
            });
        }
    }
...
document.getElementById('search-form').addEventListener('submit', function(e) {
    ...
    // 记录搜索引擎使用
    recordSearchEngineUsage(currentEngine);
