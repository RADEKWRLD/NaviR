...
function initSettingsButton() {
    ...
    function toggleSettings() {
        const isOpen = settingsBtn.classList.contains('active');
        ...
        // 如果是关闭操作，触发关闭动画
        if (isOpen) {
            settingsModal.style.animation = 'modalFadeOut 0.3s ease forwards';
            setTimeout(() => {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && settingsBtn.classList.contains('active')) {
            toggleSettings();
        }
    });
}
...
function resetToDefaultShortcuts() {
    ...
    localStorage.setItem(SHORTCUTS_STORAGE_KEY, JSON.stringify(defaultShortcuts));
    renderShortcuts();
}
...
function init() {
    ...
    document.getElementById('reset-shortcuts').addEventListener('click', () => {
        if (confirm('确定要重置所有快捷方式吗？这将恢复到默认设置。')) {
