...
// 存储搜索引擎选择的key
const SEARCH_ENGINE_KEY = 'naviR_search_engine';

// 从localStorage加载上次选择的搜索引擎
function loadSearchEngine() {
    return localStorage.getItem(SEARCH_ENGINE_KEY) || 'Google';
}

// 保存搜索引擎选择到localStorage
function saveSearchEngine(engine) {
    localStorage.setItem(SEARCH_ENGINE_KEY, engine);
}

// 当前选中的引擎，从localStorage加载
let currentEngine = loadSearchEngine();

// 初始化搜索引擎按钮文本
document.getElementById('engine-btn').textContent = currentEngine;
...
document.querySelectorAll('#engine-menu li').forEach(li => {
    li.addEventListener('click', function() {
        ...
        saveSearchEngine(name); // 保存选择到localStorage
    });
});
