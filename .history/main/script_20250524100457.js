...
function initShortcuts() {
    ...
    if (isMobileDevice()) {
        ...
        initDesktopShortcuts(grid);
    }
}

// 桌面设备的拖拽处理
function initDesktopShortcuts(grid) {
    let draggedItem = null;

    // 为所有快捷方式添加拖拽事件
    function addDragEvents(shortcut) {
        if (!shortcut.classList.contains('add-shortcut')) {
    ...
    // 初始化现有快捷方式的拖拽事件
    grid.querySelectorAll('.shortcut-item').forEach(addDragEvents);
    ...
    function handleDrop(e) {
        ...
        if (this !== draggedItem) {
            // 获取所有快捷方式
            const items = [...grid.querySelectorAll('.shortcut-item:not(.add-shortcut)')];
...
function initMobileShortcuts(grid) {
    ...
    grid.addEventListener('touchmove', (e) => {
        if (draggedItem) {
            ...
            draggedItem.style.transform = `translate3d(${currentX}px, ${currentY}px, 0)`;
            ...
            const items = [...grid.querySelectorAll('.shortcut-item:not(.dragging):not(.add-shortcut)')];
            const targetItem = items.find(item => {
                const box = item.getBoundingClientRect();
                return center.x > box.left && center.x < box.right && 
                       center.y > box.top && center.y < box.bottom;
            });
            
            if (targetItem) {
                const draggedIndex = [...grid.children].indexOf(draggedItem);
                const targetIndex = [...grid.children].indexOf(targetItem);
